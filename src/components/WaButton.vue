<script setup>
import { computed } from 'vue'
import { linkWhatsapp } from '../lib/whatsapp.js'
import { trackContact } from '../lib/pixel.js'

const props = defineProps({
  label: { type: String, default: 'Falar no WhatsApp' },
  origem: { type: String, default: 'botao' },
  // Mensagem pré-preenchida; sem ela, usa a padrão de src/data/imovel.js
  mensagem: { type: String, default: undefined }
})
const href = computed(() => linkWhatsapp(props.mensagem))
</script>

<template>
  <a class="btn-wa" :href="href" target="_blank" rel="noopener" @click="trackContact(origem)">
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path fill="currentColor" d="M17.47 14.38c-.3-.15-1.76-.87-2.03-.97-.27-.1-.47-.15-.67.15-.2.3-.77.97-.94 1.17-.17.2-.35.22-.64.07-.3-.15-1.26-.46-2.4-1.48-.89-.79-1.49-1.77-1.66-2.07-.17-.3-.02-.46.13-.61.13-.13.3-.35.45-.52.15-.17.2-.3.3-.5.1-.2.05-.37-.02-.52-.08-.15-.67-1.62-.92-2.22-.24-.58-.49-.5-.67-.51h-.57c-.2 0-.52.07-.79.37-.27.3-1.04 1.02-1.04 2.48s1.07 2.88 1.21 3.08c.15.2 2.1 3.2 5.08 4.49.71.31 1.26.49 1.69.63.71.23 1.36.2 1.87.12.57-.09 1.76-.72 2.01-1.41.25-.7.25-1.29.17-1.41-.07-.13-.27-.2-.57-.35M12.05 21.5h-.01a9.4 9.4 0 0 1-4.8-1.31l-.34-.2-3.57.93.95-3.48-.22-.36a9.4 9.4 0 0 1-1.44-5.02c0-5.2 4.23-9.43 9.44-9.43a9.4 9.4 0 0 1 9.43 9.44c0 5.2-4.24 9.43-9.44 9.43m8.03-17.46A11.3 11.3 0 0 0 12.05.72C5.8.72.7 5.8.7 12.07c0 2 .52 3.95 1.52 5.67L.6 23.62l6.02-1.58a11.3 11.3 0 0 0 5.42 1.38h.01c6.26 0 11.35-5.09 11.35-11.35 0-3.03-1.18-5.88-3.32-8.03" />
    </svg>
    {{ label }}
  </a>
</template>

<style scoped>
.btn-wa {
  display: inline-flex; align-items: center; justify-content: center; gap: 10px;
  background: var(--wa); color: #06220f; text-decoration: none;
  font-weight: 800; font-size: 17px; padding: 16px 26px; border-radius: 999px;
  box-shadow: 0 10px 30px -10px rgba(37, 211, 102, .55);
  transition: transform .15s ease, box-shadow .15s ease, background .15s ease;
}
.btn-wa:hover { transform: translateY(-1px); background: #34e077; box-shadow: 0 14px 34px -10px rgba(37, 211, 102, .7); }
svg { width: 22px; height: 22px; flex: none; }
</style>
